...
<head>
  ...
  <style>
    *, *::before, *::after {{ margin: 0; padding: 0; box-sizing: border-box; }}
    :root {{ --text: #1a1a1a; --text-secondary: #555; --bg: #fafafa; --accent: #2d6a4f; --accent-hover: #1b4332; --border: #e0e0e0; }}
    ...
    .avatar {{ width: 120px; height: 120px; border-radius: 50%; background: linear-gradient(135deg, var(--accent), #40916c); display: flex; align-items: center; justify-content: center; margin: 0 auto 2rem; font-size: 2.5rem; font-weight: 600; color: white; letter-spacing: 1px; overflow: hidden; }}
    ...
    .bio {{ font-size: 0.95rem; line-height: 1.7; color: var(--text-secondary); margin-bottom: 2.5rem; padding: 0 0.5rem; }}
    .links {{ display: flex; flex-direction: column; gap: 0.75rem; }}
    .links a {{ display: flex; align-items: center; justify-content: center; gap: 0.6rem; padding: 0.85rem 1.5rem; border: 1px solid var(--border); border-radius: 10px; text-decoration: none; color: var(--text); font-size: 0.95rem; font-weight: 500; background: white; transition: all 0.2s ease; }}
    .links a:hover {{ border-color: var(--accent); color: var(--accent); transform: translateY(-1px); box-shadow: 0 4px 12px rgba(45, 106, 79, 0.1); }}
...
def generate_index(data):
    ...
    if data.get("avatarType") == "image" and data.get("avatarImage"):
        avatar_html = f'<div class="avatar"><img src="{data["avatarImage"]}" alt="{data["name"]}"></div>'
    else:
        initials = data.get("avatarInitials", "MG")
...
class Handler(SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        elif path.startswith("/uploads/") or path.endswith((".png", ".jpg", ".jpeg", ".gif", ".webp")):
            # Serve static files
            super().do_GET()
        else:
            self.send_error(404)
    ...
    def do_POST(self):
        ...
        if path == "/api/content":
            ...
            if not boundary:
                self.send_json({"ok": False, "message": "No boundary"})
                return
...
if __name__ == "__main__":
    # Regenerate index from content on startup
    if os.path.exists(CONTENT_FILE):
        save_content(load_content())
        print(f"Regenerated index.html from content.json")
